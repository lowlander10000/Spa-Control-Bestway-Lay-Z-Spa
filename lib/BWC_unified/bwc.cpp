...
void BWC::setup(void){
    ...
    Models ciomodel = PRE2021;
    Models dspmodel = PRE2021;
    ...
    // Start zonder een automatische druk op de Power-knop.
    // Een waarde van 1 schakelde de spa bij iedere ESP-reset onbedoeld om.
    cio->cio_toggles.power_change = false;
    begin();
}
...
void BWC::_log()
{
    ...
    File file = LittleFS.open(F("/bestway_log.txt"), "a");
    if (!file) {
...
{
    File file = LittleFS.open(F("/bestway_hwcfg.json"), "r");
    if (!file)
    {
...
void BWC::_loadSettings(){
    File file = LittleFS.open(F("/bestway_settings.json"), "r");
    if (!file) {
...
void BWC::restoreStates() {
    ...
    File file = LittleFS.open(F("/bestway_states.txt"), "r");
    if (!file) {
...
void BWC::loadCommandQueue(){
    File file = LittleFS.open(F("/bestway_cmdq.json"), "r");
    if (!file) {
...
void BWC::saveRebootInfo(){
    File file = LittleFS.open(F("/bestway_bootlog.txt"), "a");
    if (!file) {
...
void BWC::_saveStates() {
    ...
    File file = LittleFS.open(F("/bestway_states.txt"), "w");
    if (!file) {
        // Serial.println(F("Failed to save states.txt"));
        return;
    }
...
void BWC::_saveCommandQueue(){
    ...
    ESP.wdtFeed();
    #endif
    File file = LittleFS.open(F("/bestway_cmdq.json"), "w");
    if (!file) {
...
void BWC::saveSettings(){
    ...
    File file = LittleFS.open(F("/bestway_settings.json"), "w");
    if (!file) {
...
void BWC::saveDebugInfo(const String& s){
    File file = LittleFS.open(F("/bestway_debug.txt"), "a");
    if (!file) {
